...
import 'package:wereads/post/editregister.dart';
import 'package:wereads/post/registeraddreciver.dart';
import 'dart:convert';

import 'package:wereads/post/registersenderadd.dart';

class RegisterPostList extends StatefulWidget {
...
class _RegisterPostListState extends State<RegisterPostList>
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      final senderResponse = await http.get(
        Uri.parse('https://api.indataai.in/wereads/fetch_senderpost.php'),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Register Posts'),
        bottom: TabBar(
          ...
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) => _tabController.index == 0
                  ? const AddRegisterPostSender()
                  : const AddRegisterPostReceiver(),
            ),
          );
        },
        child: const Icon(
...
class _EditPostScreenState extends State<EditPostScreen> {
  ...
  void initState() {
    ...
    _sAttorneyController =
        TextEditingController(text: widget.post['s_attorny']);
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      final response = await http.post(
        Uri.parse(
            'https://api.indataai.in/wereads/addregietersender.php?action=save_registerpost'),
        body: {
          ...
          's_attorny': _sAttorneyController.text,
        ...
        child: Form(
          ...
          child: ListView(
            children: [
              ...
              ElevatedButton(
                onPressed: () async {
                  await _submitForm(); // Call the form submission function
                  Navigator.pushReplacement(
                    context,
                    MaterialPageRoute(
                        builder: (context) =>
                            const RegisterpostList()), // Navigate to RegisterPostList
                  );
                },
