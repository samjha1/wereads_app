...
import 'package:wereads/post/AddcourierReceiver.dart';
import 'package:wereads/post/AddcourierSender.dart';
import 'package:wereads/post/POST_LIST.dart';
import 'package:wereads/post/editcourier.dart';
...
class _RegisterPostListState extends State<COURIERList>
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      final senderResponse = await http.get(
        Uri.parse('https://api.indataai.in/wereads/couriersender.php'),
      ...
      final receiverResponse = await http.get(
        Uri.parse('https://api.indataai.in/wereads/courierreciver.php'),
        headers: {'Content-Type': 'application/json'},
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Courier Posts'),
        bottom: TabBar(
          ...
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) => _tabController.index == 0
                  ? const AddcourierSender()
                  : const AddCourierReceiver(),
            ),
          );
        },
        child: const Icon(
