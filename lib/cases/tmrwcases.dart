...
class TomorrowCasesPage extends StatefulWidget {
  const TomorrowCasesPage({super.key});

  @override
  _NotificationPageState createState() => _NotificationPageState();
}

class _NotificationPageState extends State<TomorrowCasesPage> {
  ...
  List<Map<String, dynamic>> _filterNotificationsFor5Days(
    ...
    return notifications.where((notification) {
      ...
      return nextDate.isAfter(currentDate) &&
          nextDate.isBefore(currentDate.add(const Duration(days: 1)));
    }).toList();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Tomorrows Cases'),
